...
def minimax(state, agent, depth):
    ...
    if agent == 0:
        ...
        for a in viable_actions:
            res = minimax(transition(state, 0, a), 1, depth+1)[0]
            if res > highest_eval:
                highest_eval = res
                optimal_move = a
    ...
    # If it's the ghosts turn, minimize eval score and move onto next agent.
        ...
        # Perform search over actions with unique outcomes
        #[Note] Unlike the maximizing agent, the optimal move is not saved
        for a in viable_actions:
            if agent == ghost_count: next_agent = 0
            else: next_agent = agent + 1
            res = minimax(transition(state, agent, a), next_agent, depth+1)[0]
            if res < lowest_eval:
                lowest_eval = res
        return (lowest_eval,)
