...
def minimax(state, agent, depth, alpha=-99999999, beta=99999999):
    ...
    if agent == 0:
        ...
        #[1] Perform search over actions with unique outcomes, 
        #[2] abort search over remaining elements and prune sub-tree if
        #[3] there is a move the opponent could take that would be 
        #[4] in their favor (compared to the worst outcome found yet)
        for a in viable_actions:
            res = minimax(transition(state, 0, a), 1, depth+1, alpha, beta)[0]
            if res > highest_eval:
                highest_eval = res
                alpha = max(alpha, highest_eval)
                optimal_move = a
            if beta <= alpha:
                break
        return (highest_eval, optimal_move)
    
    # If it's the ghosts turn, minimize eval score and move onto next agent.
    elif agent >= 1:
        lowest_eval = 9999999
        unique_actions = set()
        viable_actions = []
        ...
        for a in viable_actions:
            ...
            res = minimax(transition(state, agent, a), next_agent, depth+1, alpha, beta)[0]
            if res < lowest_eval:
                lowest_eval = res
                beta = min(beta, lowest_eval)
            if beta <= alpha:
                break
        return (lowest_eval,)
...
def main():
    ...
    while True:
        ...
        best_move = minimax(game_state, 0, 0)[1]
